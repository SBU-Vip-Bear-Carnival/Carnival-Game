#pragma once
//
// Ported from V1's `cleanup` branch (main/src/track.h) on 2026-09-02,
// unchanged except for the include lines.
//
// V1 pulled its constants in transitively -- track.h included limit_switch.h,
// which included config.h. That chain is gone now that each header includes
// only what it uses, so tuning.h and types.h are named here directly.
// Reads: the TMC_* set, the speed set, TRACK_SUBDIVISIONS, TrackError.
//
// NOTE: the HardwareSerial* constructor parameter is V1's. Which port each
// track actually gets is unresolved -- V1 used Serial1 and Serial2, but V2
// gives Serial1 to the DFPlayer. That collision is deliberately left for the
// hw/uart-split PR; do not quietly change it here.
//
